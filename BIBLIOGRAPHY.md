...
| `data/strongs-es.json` | Spanish Strong's dictionary: Reina-Valera renderings derived from the tagged corpus, over Strong's own English definitions until a translation run fills them in | renderings computed by `data-prep/strongs-lang/build-strongs.py` from `data/rv1909.jsonl` | **CC-BY-SA** — a derivative of openscriptures/strongs, share-alike carries over |
