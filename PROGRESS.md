...
- **`pure-ffi` is now the real C ABI** both native shells will consume — not a
  ...
  - Cross-builds: host `.so`/`.a` ✅, **Windows x86_64 `.dll`** ✅ (mingw),
    **Windows ARM64 `.dll`** ✅ (built + tested natively on an ARM64 box).
...
## Toolchain status (updated 2026-07-14)

**ARM64 Windows is now a native platform** (built on an ARM64 Windows 11 box,
branch `windows-arm64`): rustup's default there is
`stable-aarch64-pc-windows-msvc`, and `cargo build -p pure-ffi --release`
links the ARM64 `pure_ffi.dll` natively once VS Build Tools has the
**“C++ ARM64/ARM64EC build tools”** component (`VC.Tools.ARM64`) — the SDK
alone isn't enough; without it rustc silently falls back to whatever
`link.exe` is on PATH (Git's coreutils `link`, cryptic failure). Verified on
that box: the full portable test suite (87 tests) + the R&D-feature set
(23 tests) all green, and the **C# P/Invoke demo runs against the native
ARM64 `.dll`** with the in-repo data pack
(`dotnet run --project crates/ffi/bindings/csharp/demo -- .`).

From the Linux box (status 2026-07-09): `rustup` is in and the cross
**targets are added** (android arm64/armv7/x86_64, windows x86_64-gnu +
aarch64-gnullvm); **mingw-w64** is present. So:
...
- **ARM-Windows `.dll`** from Linux needs **llvm-mingw**
  (`aarch64-w64-mingw32-clang`) for the `aarch64-pc-windows-gnullvm` target —
  moot for release builds now that the ARM64 box builds it natively.
...
9. **Native shell apps over the ABI**:
   ...
   - **Windows (WinUI): ~~started~~ — first milestone SHIPPED** (2026-07-14,
     ARM64 box, branch `windows-arm64`):
     [`apps/windows/PureStudyWin`](apps/windows/PureStudyWin/README.md), a
     code-only WinUI 3 + Win2D shell over the P/Invoke bindings. Renders
     chapters from the core's display list (EB Garamond, gold verse numbers,
     italic added-words, Strong's underlines), full keyboard/wheel nav + zoom,
     double-click word study (Strong's + concordance jump links), and
     multi-tier search with verse-highlight jumps. Verified rendering + word
     study on-screen; remaining gaps vs. GTK are listed in its README.

## Session note

Set Hyprland window rules (float/center) for class `dev.purestudy.app` (renamed from the old cavallo id) to grab
