...
config save refreshes the CURRENT seating's slot from the live primary pane,
first-visible verse included (2026-08-21): the slot wins the restore, so when
it was marked chapter-only at navigation time it beat the verse-carrying
`openPanes` and every same-seating reopen landed at the top of the chapter.
Pinned by `e2e/session-slots.spec.ts` ("restores the scroll position").
