...
pub fn layout_chapter<M: Measure>(verses: &[Verse], m: &M, cfg: &LayoutConfig) -> DisplayList {
    ...
    for verse in verses {
        ...
        if pen.line_started && pen.x + num_w > cfg.width {
            pen.newline(cfg);
        }
        ...
        for (ti, token) in verse.tokens.iter().enumerate() {
            ...
            if token.has_flag(FLAG_PARA) && pen.line_started {
...
mod tests {
    ...
    fn paragraph_flag_breaks_line() {
        ...
        let dl = layout_chapter(&verses, &m, &cfg);
        // The "Thus" word (start of Gen 2:1) should sit at the paragraph indent.
        let thus = dl.items.iter().find(|it| it.text == "Thus").unwrap();
        assert_eq!(thus.x, cfg.para_indent);
        assert!(thus.y > 0.0);
    }
}
