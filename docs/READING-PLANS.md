# Reading plans + the speedrun — design spec
...
| 6 | Speedrun | A new plan **kind** with its own reader mode — see below. |
...
```json
{
  ...
  "kind": "schedule",           // "schedule" | "speedrun"
  "class": "wholeBible",        // exclusivity group; absent on a speedrun
  ...
  "tag": null                    // speedrun only: the preset tag name
}
```
...
## Speedrun — a concept sweep with its own reader mode
...
and study them together."* Tags → weave already exist; the speedrun is the
sweep that feeds them.

- **Its own mode, clicked into.** Starting (or resuming) a speedrun puts the
  reader in speedrun mode — a visible banner/chip names the run and its tag,
  and the mode persists until the reader switches back. Only one speedrun is
  active in the reader at a time (the mode changes tap semantics; two at once
...
- **The tracker is OFF while in the mode.** Skimming is not reading: the
  ...
  schedule-plan derived from it, ignores speedrun time entirely.
...
- **Tap-to-tag.** In speedrun mode a verse tap tags the verse with the run's
...
- **Done.** A speedrun has no finish line to enforce — the reader ends it from
...
- `plumbline_engine_speedrun_state_json()` / `_sweep(id, book, chapter)` /
  `_set_mode(id_or_null)` — the mode is engine state like the AKJV overlay,
  so two panes cannot disagree about what a tap means.
...
2. `plumbline_core::plan`: generator, classes, plan file store, derived
   completion, speedrun record. Unit tests over a toy corpus.
3. Chronological table: data-prep script + pack entry (study stage) + loader.
4. FFI endpoints + bindgen + the golden ABI tests.
5. Web shell: Plans screen, navigator card, nav chip, speedrun mode +
