...
//
// This file is the low-level [PureStudyNative] interface only — it mirrors
// every function in `crates/ffi/include/pure_study.h` (87 of them). The safe
// wrappers (StudyEngine / Chapter / StudyConfig) live in the Compose shell
// (`apps/android/app/src/main/java/dev/purestudy/StudyEngine.kt`), PascalCase to
...
//
// The .so is built with cargo-ndk into jniLibs/{arm64-v8a,x86_64}/libpure_ffi.so
 ...
 *  [PureStudyNative.pure_study_string_free]; opaque handles are [Pointer] and
...
internal interface PureStudyNative : Library {
    ...
    fun pure_engine_memory_grade(
        engine: Pointer, verseRef: String, grade: String, now: String,
    ): Pointer?
    fun pure_engine_memory_add(engine: Pointer, verseRef: String, now: String): Pointer?
    fun pure_engine_memory_remove(engine: Pointer, verseRef: String): Pointer?
    fun pure_engine_memory_card_json(engine: Pointer, verseRef: String): Pointer?
    fun pure_engine_memory_due_json(engine: Pointer, now: String): Pointer?
    fun pure_engine_memory_coverage_json(engine: Pointer, now: String): Pointer?
    fun pure_engine_memory_activity_json(engine: Pointer): Pointer?
    fun pure_engine_memory_drill_json(engine: Pointer, verseRef: String, level: Int): Pointer?
    fun pure_engine_memory_score_json(engine: Pointer, verseRef: String, typed: String): Pointer?
    ...
    companion object {
        val INSTANCE: PureStudyNative = Native.load("pure_ffi", PureStudyNative::class.java)
    }
}
